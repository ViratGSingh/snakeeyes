...
class User(UserMixin, ResourceMixin, db.Model):
    ...
    def deserialize_token(cls, token):
        ...
        return User.find_by_identity(decoded_payload.get('email'))
    ...
    def initialize_password_reset(cls, identity):
        ...
        serializer.dumps({"x": 42})
        data = {"email":str(u.email),"id":str(u.id), "token":serializer.dumps({"x": 42}),"password":md5(u.password.encode('utf-8')).hexdigest()}
